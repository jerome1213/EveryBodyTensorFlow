...
def get_model(model_in, dropout_keeprate_node, train_config, scope):
    ...
    with tf.variable_scope(name_or_scope=scope, values=[model_in]):
        with slim.arg_scope([slim.conv2d], padding='SAME', stride=[2, 2], kernel_size=[5, 5]):
            with slim.arg_scope([slim.max_pool2d], padding='VALID', stride=[1, 1], kernel_size=[5, 5]):
                c1_out = slim.conv2d(inputs=net, num_outputs=6, stride=[1, 1], scope='c1_conv')
                s2_out = slim.max_pool2d(inputs=c1_out, kernel_size=[2, 5], scope='s2_pool')
